...
(define-syntax number-parameter
  (lambda (stx)
    (raise-syntax-error 'number-parameter "Invalid use outside path pattern" stx)))

(define-syntax string-parameter
  (lambda (stx)
    (raise-syntax-error 'string-parameter "Invalid use outside path pattern" stx)))

(provide number-parameter string-parameter)

(syntax-spec
  ;; Binding classes for pattern variables
  ...
  ;; A route clause: method, path, and an action expression.
  (nonterminal route
               #:binding-space route-binding
  ...
  ;; Host interface for the router DSL.
...
(define (compile-routes-clauses routes)
  ...
  (check-equal?
 (map syntax->datum
      (compile-routes-clauses #'((get [blog (number-parameter article-no)] (list 'numbered article-no)))))
 '(((list 'get (list "blog" (app string->number (? number? article-no))))
    ...
    (check-not-exn
     (λ ()
       (expand-syntax
        #'(router 'get '()
                (get [blog (number-parameter article-no)] 'ok)))))

    (check-false
     (first-unreachable-route
      (list #'(get [blog (number-parameter article-no)] 'ok)
             #'(get [blog (string-parameter name)] 'ok)))
     "Routes should be reachable")
 ...
 (λ ()
   (expand-syntax
    #'(router 'get '("blog")
            (get [blog] 'list-articles)
            (get [blog (number-parameter article-no)] (list 'numbered article-no))))))
 ...
 (λ ()
   (expand-syntax
    #'(router 'get '("blog")
            (get [blog] 'list-articles)
            (get [blog (number-parameter article-no)] (list 'numbered article-no))
            (get [blog (string-parameter article-name)] (list 'named article-name))))))

    (test-case "Inspect test-router expansion"
  (with-handlers ([exn:fail:syntax?
                   (λ (exn)
                     (displayln (format "Syntax error: ~a" (exn-message exn)))
                     (void))])
    (define expanded
      (expand-syntax
       #'(router 'get '("blog")
               ...
               (get [blog latest] 'latest)
               (get [blog (string-parameter article-name)] (list 'named article-name)))))
    (displayln "Expanded syntax:")
    (displayln (syntax->datum expanded))))

    (test-case "Single route with number-parameter expands correctly"
  (check-equal?
   (syntax->datum
    (expand-syntax
     #'(router 'get '("blog")
   ...
   '(match (list 'get '("blog"))
    ...
    (test-case "Multiple routes expand correctly"
  (check-equal?
   (syntax->datum
    (expand-syntax
     #'(router 'get '("blog")
   ...
   '(match (list 'get '("blog"))
    ...
    (test-case "Invalid route syntax raises error"
  (check-exn
   exn:fail:syntax?
   (λ ()
     (expand-syntax
      #'(router 'get '("blog")
                (get (blog) 'list-articles))))
   "Should fail due to incorrect path syntax"))
